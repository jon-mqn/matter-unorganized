import type { Board, BlockDef, Rng } from "./types.js";
...
export function validateSizes(sizes: number[]): void {
  if (sizes.length < 1) {
  ...
  for (const s of sizes) {
    ...
    if (s < MIN_SIZE || s > MAX_SIZE) {
      throw new Error(
        `Square size ${s} must be between ${MIN_SIZE} and ${MAX_SIZE}.`,
      );
    }
  }
  ...
  if (cells > MAX_CELLS) {
    throw new Error(
      `Board is too large (${cells} cells; max ${MAX_CELLS}). Use fewer or smaller squares.`,
    );
  }
}
 ...
 */
export function generationBudget(cellCount: number): {
  passes: number;
  attempts: number;
  timeBudgetMs: number;
} {
  ...
  if (cellCount <= 340) return { passes: 4, attempts: 8, timeBudgetMs: 7000 };
 ...
 */
export function stackedBlockDefs(sizes: number[]): BlockDef[] {
  validateSizes(sizes);
  const lefts: number[] = [];
  const tops: number[] = [];
...
export function stackedBoardId(sizes: number[]): string {
  return sizes.join("-");
}

const stackedCache = new Map<string, Board>();

/** Build (and cache) a vertical Frankengrid board from square sizes. */
export function buildStackedBoard(sizes: number[]): Board {
 ...
 */
export function arrangeBlocks(sizes: number[], arrSeed: number): BlockDef[] {
  validateSizes(sizes);
  ...
  const placed: Rect[] = [{ r: 0, c: 0, s: sizes[0]! }];
  ...
  const rels: ((a: Rect, s: number) => Rect | null)[] = [
  ...
  function rel(
    a: Rect,
    s: number,
    kind: "below" | "above" | "right" | "left" | "overlap",
    r: Rng,
  ): Rect | null {
    switch (kind) {
      case "below": {
  ...
  function addsCells(cand: Rect, all: Rect[]): boolean {
  ...
  function couplesAny(cand: Rect, all: Rect[]): boolean {
    ...
    return all.some((p) => {
      ...
      if (ro > 0 && co > 0) return true; // overlap: shares cells
  ...
  function evenBetween(lo: number, hi: number, r: Rng): number | null {
...
export function parseArrSeed(token: string): number | undefined {
 ...
 */
export function parseBoardToken(token: string): number[] | null {
  const base = token.split("~")[0]!;
  if (base === "reference") return [6, 6];
  const sizes = base.split("-").map((p) => Number(p));
  if (sizes.some((n) => !Number.isInteger(n))) return null;
  try {
    validateSizes(sizes);
  } catch {
    return null;
...
export const BOARD_REGISTRY: Record<string, () => Board> = {
