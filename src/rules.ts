...
export function lineViolatesNoThree(state: State, line: Line): boolean {
  const cells = line.cells;
  for (let i = 0; i + 2 < cells.length; i++) {
    const a = state[cells[i]!]!;
    if (a === EMPTY) continue;
    if (a === state[cells[i + 1]!] && a === state[cells[i + 2]!]) return true;
  }
  return false;
}
...
function windowViolationTouching(
  state: State,
  line: Line,
  cellIdx: number,
): boolean {
  ...
  for (let start = pos - 2; start <= pos; start++) {
    if (start < 0 || start + 2 >= cells.length) continue;
