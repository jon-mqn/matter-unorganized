...
function reduceOnce(solution, board, rng, keep) {
    ...
    for (const id of ids) {
        ...
        clues.delete(id);
        if (!keep(clues))
            clues.set(id, val); // removal broke the invariant; restore
    }
    return clues;
}
/** Run `run` over N passes with independent derived RNGs; keep fewest clues. */
function multiPass(passes, rng, run) {
    let best;
    for (let p = 0; p < passes; p++) {
        const sub = makeRng(Math.floor(rng() * 0x100000000));
        const clues = run(sub);
        if (best === undefined || clues.size < best.size)
            best = clues;
    }
    return best;
}
/** Uniqueness-minimal reduction: keep the puzzle's solution unique (§4.6). */
export function reduceUnique(solution, board, rng, passes = DEFAULT_PASSES) {
    return multiPass(passes, rng, (r) => reduceOnce(solution, board, r, (clues) => countSolutions(clues, board, 2) === 1));
}
/**
 ...
 */
export function reduceLogic(solution, board, rng, maxTier, passes = DEFAULT_PASSES) {
    return multiPass(passes, rng, (r) => reduceOnce(solution, board, r, (clues) => isFullySolved(clues, board, maxTier)));
}
